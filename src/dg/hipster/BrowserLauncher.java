 ...
 * application, applet, or other binary that makes use of this code, but
 ...
 * of data or functionality or any adverse or unexpected effects of using this
 * software.
 * <p>
 * Credits:
        ...
        if (osName.startsWith("Mac OS")) {
            ...
            try {
                ...
                if (version == 2) {
                    ...
                    // supports that but has an openURL() method as well that
                    // we currently ignore.
                    jvm = MRJ_2_1;
                } else if (version == 3.0) {
                    jvm = MRJ_3_0;
            ...
            case MRJ_2_0:
                try {
                    ...
                    Field anyTransactionIDField
                            = appleEventClass.getDeclaredField(
                            "kAnyTransactionID");
                    kAnyTransactionID = (Integer) anyTransactionIDField.get(
                            null);
        ...
        switch (jvm) {
            case MRJ_2_0:
                try {
                    ...
                    // Don't set browser = appleEvent because then the next
                    // time we call locateBrowser(), we'll get the same
                    // AppleEvent, to which we'll already have added the
                    // relevant parameter. Instead, regenerate the AppleEvent
                    // every time.
            ...
            case MRJ_2_1:
                ...
                // Avoid a FilenameFilter because that can't be stopped
                // mid-list
                for(int i = 0; i < systemFolderFiles.length; i++) {
                    try {
                        ...
                        // 'MACS' and a type of 'FNDR'.  Only requiring the
                        // type results in non-Finder applications being picked
                        // up on certain Mac OS 9 systems, especially German
                        // ones, and sending a GURL event to those applications
        ...
        switch (jvm) {
            case MRJ_2_0:
                ...
                try {
                    ...
                    throw new IOException(
                            "IllegalAccessException while building "
